...
Every line the Pod writes is `tracing`, on stdout, JSON by default and `text` when `logFormat` says so. That includes a startup failure before the config is read, which uses the default JSON logger rather than a bare stderr write. One format means one log query.

```json
{"timestamp":"2026-07-27T02:14:09.113Z","level":"WARN","vpn_connection":"prod-dc (vpn-0123456789abcdef0)","message":"*Not replacing.* ...","target":"replacement alert"}
```

The `validate` and `status` subcommands are the exception. They print tables to stdout for a human running `kubectl exec`, and are not part of the Pod's log stream.
...
```bash
make fmt lint test   # rustfmt, clippy -D warnings, cargo test
make coverage        # fails below 70% line coverage (cargo llvm-cov)
...
The container image is `scratch` plus the CA bundle. The binary is statically linked with cargo-zigbuild for `x86_64-unknown-linux-musl` and `aarch64-unknown-linux-musl`, and the timezone database is compiled in, so the image carries no `/usr/share/zoneinfo`.

The chart README is generated by helm-docs and will be overwritten; edit `charts/aws-vpn-maintenance-handler/values.yaml` instead.
