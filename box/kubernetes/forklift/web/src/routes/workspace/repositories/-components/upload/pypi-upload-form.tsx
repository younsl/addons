...
export function PyPIUploadForm({ repository, csrfToken }: { repository: Repository; csrfToken?: string }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    setError("");
    setResult(null);
    if (!file || !/\.(whl|tar\.gz|zip)$/i.test(file.name)) {
      setError(t("upload.pypi-files-required"));
      return;
    }
    const form = new FormData();
    form.append("manifest", JSON.stringify({
      schema_version: 1, format: "pypi", overwrite: false,
      assets: [{ part: "asset0" }], pypi: {},
    }));
    ...
    try {
      const uploaded = await uploadArtifact(repository.id, form, {
        idempotencyKey: idempotencyKey.current, csrfToken, signal: ctl.signal,
    ...
    <div className="min-w-0 max-w-4xl">
      ...
      {result ? (
        <Card size="sm"><CardContent><div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1"><h2 className="m-0 text-base font-semibold">{t("upload.success")}</h2>
            <p className="mt-1 text-sm text-muted-foreground">{result.coordinate}</p>
            <p className="text-sm text-muted-foreground">{t("upload.created")}: {result.created.length}</p>
            <Link className={buttonVariants({ className: "mt-3" })} to="/workspace/repositories/$id/$tab"
              params={{ id: String(repository.id), tab: "artifacts" }}>{t("upload.view-artifacts")}</Link>
          </div>
        </div></CardContent></Card>
      ) : (
        <form onSubmit={submit}>
          <Card size="sm" className="mb-4"><CardContent>
            <h2 className="m-0 mb-1 text-base font-semibold">{t("upload.pypi-release")}</h2>
            <p className="mb-4 mt-0 text-sm text-muted-foreground">{t("upload.pypi-help")}</p>
            <Field><FieldLabel htmlFor="upload-pypi-file">{t("upload.file")}</FieldLabel>
              <Input id="upload-pypi-file" type="file" accept={repository.config.upload?.pypi_allow_legacy_zip ? ".whl,.tar.gz,.zip" : ".whl,.tar.gz"} disabled={busy}
                onChange={(event) => { setFile(event.target.files?.[0] ?? null); idempotencyKey.current = null; }} />
              <FieldDescription>{file ? `${file.name} · ${formatFileSize(file.size)}` : t("upload.pypi-files-help")}</FieldDescription>
            </Field>
          </CardContent></Card>
