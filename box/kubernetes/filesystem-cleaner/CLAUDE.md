...
Filesystem cleaner for Kubernetes, run as an init container (`once` mode) or sidecar (`interval` mode). Standard library only. Do not add third-party dependencies without a strong reason.
...
- CI gates on formatting, vet, race-enabled tests, and 70% line coverage. Release triggers are documented in the repo root CLAUDE.md.
