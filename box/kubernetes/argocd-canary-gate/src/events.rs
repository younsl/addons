...
const COMPONENT: &str = "argocd-canary-gate";
...
pub const REASON_BLOCKED: &str = "CanaryBlocked";
/// Marks a sync the gate allowed while recording something about it.
pub const REASON_WARNED: &str = "CanaryWarning";
...
/// the API server an object whose size depends on how many Rollouts an
/// Application happens to manage.
            ...
            while let Some(event) = rx.recv().await {
                ...
                match sink.create(event).await {
                    ...
                    Err(err) => tracing::warn!(
                        ...
                        reason = %reason,
                        error = %err,
                        "could not write a canary gate event. The verdict itself was unaffected"
                    ),
                }
            }
        });
...
mod tests {
    ...
    async fn recorder_writes_blocked_and_warned_verdicts_only() {
        ...
        assert!(
            text.contains(r#"events_total{reason="CanaryBlocked",type="Warning"} 1"#),
            "{text}"
        );
        assert!(
            text.contains(r#"events_total{reason="CanaryWarning",type="Normal"} 1"#),
            "{text}"
        );
    }
