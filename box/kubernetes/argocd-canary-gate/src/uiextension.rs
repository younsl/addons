...
pub const DEFAULT_NAME: &str = "canary-gate";
...
pub const FILE_NAME: &str = "extensions-CanaryGate.js";

/// The per-extension directory Argo CD expects inside `<extensions>/resources`.
pub const DIR_NAME: &str = "extension-CanaryGate.js";
...
/// `<root>/resources/extension-CanaryGate.js/extensions-CanaryGate.js`.
...
mod tests {
    ...
    fn tar_is_deterministic_and_carries_the_installer_layout() {
        let first = tar("canary-gate").unwrap();
        let second = tar("canary-gate").unwrap();
        ...
        assert_eq!(
            ...
            "resources/extension-CanaryGate.js/extensions-CanaryGate.js"
        );
        assert_eq!(entry.header().mode().unwrap(), 0o644);
        assert_eq!(entry.header().mtime().unwrap(), 0);
        assert_eq!(
            entry.header().size().unwrap(),
            script("canary-gate").unwrap().len() as u64
        );
