...
# an unpleasant surprise waiting to happen. A dedicated file per cluster means
# the real config is never opened, let alone modified.
...
# argocd-canary-gate-linux-<arch> in the build context, the same layout the
...
build_and_load_image() {
  ...
  (cd "${REPO_ROOT}" &&
    ...
    cp "target/${target}/release/argocd-canary-gate" "argocd-canary-gate-linux-${arch}")

  log "building and loading ${IMAGE}"
  "${BUILDER}" build --platform "linux/${arch}" -t "${IMAGE}" "${REPO_ROOT}"
  local archive
  archive="$(mktemp -t acg-image-XXXXXX).tar"
