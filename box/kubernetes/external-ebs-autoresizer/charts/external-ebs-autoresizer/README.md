...
| config.ssmCommandTimeout | string | `"5m"` | SSM command poll timeout as a duration (30s, 5m, 1h) |
| config.ssmPollInterval | string | `"1s"` | Delay between SSM command and volume modification status polls as a duration (30s, 5m, 1h) |
| config.volumeModifyTimeout | string | `"10m"` | ModifyVolume optimizing-wait timeout as a duration (30s, 5m, 1h) |
...
| config.alertmanager.timeout | string | `"5s"` | Timeout for each Alertmanager POST as a duration (30s, 5m, 1h) |
...
| config.grafanaAnnotation.timeout | string | `"5s"` | Timeout for each Grafana annotation POST as a duration (30s, 5m, 1h) |
...
| config.throughputRecommendation.lookbackWindow | string | `"7d"` | How far back the observation window reaches, as a Prometheus duration (7d, 12h), unlike the other intervals. Bounded by the metrics backend's retention. |
| config.throughputRecommendation.interval | string | `"30m"` | How often recommendations are recomputed, as a duration (30s, 5m, 1h). Separate from reconcileInterval and higher on purpose: the observation window spans days, so a much shorter interval only re-runs the most expensive query the addon issues. |
