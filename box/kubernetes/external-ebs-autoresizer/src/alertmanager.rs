...
impl Client {
    ...
    /// Performs a one-time liveness check against the Alertmanager health
    /// endpoint. It never mutates state; the caller decides what to do with a
    /// failure.
...
mod tests {
    ...
    async fn preflight_outcomes() {
        ...
        assert_eq!(p.endpoint, format!("{}/-/healthy", server.uri()));
        ...
        assert!(err.starts_with("get http://127.0.0.1:1/-/healthy"), "{err}");
    }
