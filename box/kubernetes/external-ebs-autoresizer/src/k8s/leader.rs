//! Single-active-instance leader election backed by a `coordination.k8s.io`
//! Lease. It lets the Deployment run multiple replicas for high availability
//! while guaranteeing that only the leader reconciles, which avoids concurrent
//! `ModifyVolume` calls against the same EBS volume.
...
/// Lease timing. These mirror the common controller defaults: the leader
/// renews every `retry_period` and must succeed within `renew_deadline`; a
/// standby waits out `lease_duration` before taking over.
...
pub struct LeaderConfig {
    ...
    pub identity: String,
    /// Holds the Lease object; use the Pod namespace.
    pub namespace: String,
    /// The Lease object name shared by all candidates.
...
/// to `lead` is cancelled the moment leadership is lost, so the reconcile
/// loops stop promptly; the process then re-contends rather than exiting.
...
where
    ...
    Fut: Future<Output = ()>,
{
    if cfg.identity.is_empty() || cfg.namespace.is_empty() || cfg.lease_name.is_empty() {
        return Err(LeaderError::Incomplete);
    }
    info!(
        lease_namespace = %cfg.namespace,
        ...
        match try_acquire(api, cfg, Utc::now()).await {
            Ok(Acquire::Acquired) => {
                info!(
                    ...
                    lease_name = %cfg.lease_name,
                    identity = %cfg.identity,
                    "acquired leadership, starting reconcile loop"
                );
                ...
                info!(
                    ...
                    lease_name = %cfg.lease_name,
                    identity = %cfg.identity,
                    "lost leadership, stopping reconcile loop"
                );
                if shutdown.is_cancelled() {
...
mod tests {
    ...
    async fn rejects_incomplete_config() {
        ...
        for c in [
            ...
            {
                let mut c = cfg("pod");
                c.lease_name = String::new();
                c
            },
        ] {
            let err = run(&api, &c, CancellationToken::new(), |_| async {})
                .await
                .unwrap_err();
    ...
    async fn acquires_leads_and_releases_on_shutdown() {
        ...
        assert_eq!(lease.metadata.namespace.as_deref(), Some("kube-system"));
