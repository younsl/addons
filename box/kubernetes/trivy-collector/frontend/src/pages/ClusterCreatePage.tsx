...
const CLUSTERS_PATH = '/admin/clusters'
...
/**
 * Standalone registration wizard at /admin/clusters/new.
 *
 * Lives on its own route so the Clusters list stays a list and the wizard is
 * deep-linkable. Finishing (or cancelling) returns to the Clusters page.
 */
      ...
      <div className={styles.section}>
        <div className={styles.sectionHeader} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <button
            ...
            onClick={() => navigate(CLUSTERS_PATH)}
            title="Back to Clusters"
          >
            <i className="fa-solid fa-arrow-left" /> Clusters
          </button>
        ...
        {step === 1 ? (
          ...
          <Step2Register
            ...
            edgeNamespace={edgeNamespace}
            onBack={() => setStep(1)}
            onSuccess={() => navigate(CLUSTERS_PATH)}
          />
        )}
      </div>
    </div>
