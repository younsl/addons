...
const CreateTokenForm = ({ settings }: { settings: PatSettings }) => {
  const api = useApi(patApiRef);
  const navigate = useNavigate();
  const todayDate = useMemo(() => today(getLocalTimeZone()) as unknown as DateValue, []);
    ...
    return (
      <Flex direction="column" gap="3" style={{ maxWidth: 720 }}>
        ...
        <Flex gap="2">
          <Button
            variant="primary"
            onPress={() => navigate(`/pat/tokens/${encodeURIComponent(created.record.id)}`)}
          >
            Open token
          </Button>
          <Button variant="secondary" onPress={() => navigate('/pat')}>
            Back to tokens
          </Button>
        </Flex>
      </Flex>
  ...
  return (
    <Flex direction="column" gap="4" style={{ maxWidth: 720 }}>
      <TextField
        ...
        description={`Letters, digits, - and _ only. Fixed after creation. ${trimmedName.length}/${NAME_MAX}`}
        value={name}
      ...
      <ScopeGrid plugins={settings.scopablePlugins} access={access} onChange={setAccess} />
      ...
      <Flex gap="2">
        ...
        <Button variant="secondary" onPress={() => navigate('/pat')} isDisabled={submitting}>
          Cancel
        </Button>
      </Flex>
    </Flex>
...
export const CreateTokenPage = () => {
  const api = useApi(patApiRef);
  const { value: settings, loading, error } = useAsync(() => api.getSettings(), [api]);
  ...
  return (
    <Container my="4">
      <Flex direction="column" gap="4">
        ...
        {error && (
          <Alert status="danger" title="Failed to load token settings" description={error.message} />
        )}
        {settings && <CreateTokenForm settings={settings} />}
      </Flex>
    </Container>
  );
};
